...
import {
  durableHistoryState,
  hydrateHistorySnapshot,
} from "../canvas/history/serialization.js";
...
} from "../workflows/templates/catalog.js";
...
const LOCKED_CONNECTION_ACTIVITY =
  "Unlock locked nodes before changing their connections.";
...
function workflowDecisionIsCurrent(
  ...
  if (execution === null || target.request.executionId !== execution.id)
    return false;
...
const WorkspaceInner = forwardRef<WorkspaceHandle, WorkspaceProps>(
  function WorkspaceInner(
    ...
    const [workflowDecision, setWorkflowDecision] =
      useState<WorkflowDecisionTarget | null>(null);
    ...
    useEffect(() => {
      ...
      const loadWorkspace = async () => {
        if (typeof window.forgeboard.canvas.loadWithHistory === "function") {
          return unwrap(
            await window.forgeboard.canvas.loadWithHistory(project.id),
          );
        }
        const document = unwrap(
          await window.forgeboard.canvas.load(project.id),
        );
        return {
          document,
    ...
    useEffect(() => {
      return window.forgeboard.runs.onEvent((event) => {
        ...
        setNodes((items) =>
          items.map((node) => {
            ...
            const transcript = update.transcript
              ? `${node.data.transcript ?? ""}${update.transcript}`.slice(
                  -100_000,
                )
                ...
                ...(update.interactiveInputSupported === undefined
                  ...
                  : {
                      interactiveInputSupported:
                        update.interactiveInputSupported,
                    }),
        ...
        if (update.activity) {
          setEvents((items) =>
            [update.activity as string, ...items].slice(0, 80),
          );
        }
      });
    }, []);
    ...
    const pendingHistory = useMemo(
      () =>
        canvas === null
          ? null
          : durableHistoryState(project.id, canvas.id, past, future),
    ...
    const applyCollaborationSnapshot = useCallback(
      (
        ...
        const merged = mergeCollaborationCanvasSnapshot(
          pendingCanvas,
          snapshot,
          context,
        );
        ...
        const nextSelectedNodeId = merged.document.nodes.some(
          (node) => node.id === selectedNodeId,
        )
          ? selectedNodeId
          : null;
        const nextSelectedEdgeId = merged.document.edges.some(
        ...
        setEvents((items) =>
          items[0] === "Applied the latest shared canvas changes."
            ? items
            : ["Applied the latest shared canvas changes.", ...items].slice(
                0,
                80,
              ),
        );
    ...
    const initializeGit = useCallback(async () => {
      ...
      try {
        const updated = unwrap(
          await window.forgeboard.projects.initializeGit(project.id),
        );
        if (updated) {
          await onProjectUpdated(updated);
          setEvents((items) =>
            ["Set up Git without changing any existing files.", ...items].slice(
              0,
              30,
            ),
          );
        ...
        onError(
          cause instanceof Error ? cause.message : "Git could not be set up.",
        );
      } finally {
    ...
    const undo = useCallback(() => {
      ...
      setEvents((items) =>
        ["Undid the last canvas change.", ...items].slice(0, 30),
      );
    }, [
        ...
        if (result.movedNodeIds.length > 0) {
          ...
          const affectedFrameIds = [
            ...
            ...currentNodes
              .filter(
                (node) =>
                  node.data.kind === "group-frame" && movedNodeIds.has(node.id),
              )
              .map((node) => node.id),
          ];
    ...
    const addNode = useCallback(
      (
        ...
        const titlesInUse = new Set(
          currentNodes.map((node) => node.data.title),
        );
        ...
        window.setTimeout(() => {
          if (pendingNodeSelection.current === id)
            pendingNodeSelection.current = null;
        }, 250);
        setEvents((items) =>
          [`Added ${definition.label} node.`, ...items].slice(0, 30),
        );
      },
      [
        collaborationCanvas.graphReadOnly,
    ...
    const addExtensionNode = useCallback(
      (template: ExtensionTemplate, position?: { x: number; y: number }) => {
        ...
        const titlesInUse = new Set(
          nodesRef.current.map((node) => node.data.title),
        );
        pendingNodeSelection.current = id;
        setNodes((items) => [
          ...items.map((node) => ({ ...node, selected: false })),
        ...
        setEvents((items) =>
          [`Added ${definition.displayName} extension node.`, ...items].slice(
            0,
            30,
          ),
        );
      ...
      (template: WorkflowTemplate) => {
        ...
        try {
          ...
          if (selected === undefined)
            throw new Error("The workflow template contains no nodes.");
          record();
          ...
          window.setTimeout(() => {
            if (pendingNodeSelection.current === selected.id)
              pendingNodeSelection.current = null;
          }, 250);
          setEvents((items) =>
            [
    ...
    const attachProjectFileContext = useCallback(
      async (
        ...
        if (lockedCanvasNodeIds(nodesRef.current).has(targetNodeId)) {
          throw new Error(
            "Unlock the Agent node or its group before changing its context.",
          );
        }
        ...
        if (!result.changed) {
          setEvents((items) =>
            ["That project file is already attached.", ...items].slice(0, 30),
          );
          return;
        }
    ...
    const removeProjectFileContext = useCallback(
      (targetNodeId: string, attachmentNodeId: string): void => {
        ...
        setNodes(result.nodes);
        setEvents((items) =>
          ["Removed a project file from the agent's context.", ...items].slice(
            0,
            30,
          ),
        );
    ...
    const selectedAdapter = selectedNode
      ...
      : "test-agent";
    const selectedAgent = runnableAgents.find(
      (agent) => agent.id === selectedAdapter,
    );
    ...
    const watchedProviderAdapterIds = useMemo(() => {
      ...
      for (const node of nodes) {
        if (node.data.kind === "agent")
          ids.add(node.data.adapterId ?? fallbackAdapter);
      }
    ...
    const agentRunBlockReason = useCallback(
      (node: WorkshopNode): string | null => {
        ...
        const gate = providerGates.gateFor(
          node.data.adapterId ?? fallbackAdapter,
        );
    ...
    const canRunWorkflow =
      runnableWorkflowNodeCount(nodes, edges, agentRunBlockReason) > 0;

    const updateNodeData = useCallback(
      (nodeId: string, data: Partial<WorkshopNode["data"]>) => {
        setNodes((items) => {
    ...
    const workflowInteractiveNodeIds = useMemo(
      () =>
        new Set(
          nodes
            .filter(
              (node) => node.data.kind === "agent" || node.data.kind === "task",
            )
            .map((node) => node.id),
        ),
      [nodes],
      ...
      () =>
        new Map(
          (workflowEvidenceExecution?.nodeRuns ?? []).map(
            (run) =>
              [run.nodeId, workflowCanvasNodeStatus(run.status)] as const,
          ),
        ),
    ...
    const workflowReviewGates = useMemo(
      () =>
        new Map(
          (workflowEvidenceExecution?.reviewGates ?? []).map(
            (gate) => [gate.nodeId, gate] as const,
          ),
        ),
      [workflowEvidenceExecution],
    );
    ...
    const runtimeDisplayedNodes = useMemo(
      () =>
        nodes.map((node) => {
          ...
          const inheritedLock =
            protectedNodeIds.has(node.id) && !node.data.locked;
          const displayed =
              ...
              : {
                  ...
                  data: {
                    ...
                    ...(reviewGate === undefined
                      ? {}
                      : {
                          gateState: workflowCanvasReviewGateState(
                            reviewGate.status,
                          ),
                        }),
    ...
    const workflowEdgeStates = useMemo(
      () =>
        new Map(
          (workflowEvidenceExecution?.edges ?? []).map((edge) => [
            edge.edgeId,
            edge,
          ]),
        ),
    ...
    const filteredExtensionTemplates = extensionTemplates.filter(
    ...
    const { arrangeGroupFrame, deleteNode, fitGroupFrame, setNodeLocked } =
    ...
    function updateEdgeData(data: WorkshopEdgeData) {
      ...
      record();
      setEdges((items) =>
        updateWorkspaceEdgeData(items, selectedEdge.id, data),
      );
    }
    ...
    const promptAgentFromVoice = useCallback(
      (nodeId: string, prompt: string) => {
        ...
        if (
          ...
          onError(
            "Unlock the Agent and ensure the canvas is editable before prompting it.",
          );
          return;
        }
        if (
          ...
          void window.forgeboard.runs
            ...
            .then(() => {
              setEvents((items) =>
                [
                  `Sent a voice-confirmed prompt to ${target.data.title}.`,
                  ...items,
                ].slice(0, 30),
              );
            })
            .catch((cause: unknown) => {
              onError(
                cause instanceof Error
                  ? cause.message
                  : "Could not prompt the running Agent.",
              );
            });
          return;
        }
        ...
        const nextNodes = nodesRef.current.map((node) => ({
          ...
          data:
            node.id === nodeId
              ? applyNodeDataPatch(node.data, { prompt })
              : node.data,
        }));
        nodesRef.current = nextNodes;
        setNodes(nextNodes);
    ...
    const connectNodesFromVoice = useCallback(
      (sourceNodeId: string, targetNodeId: string) => {
        ...
        if (alreadyConnected) {
          setEvents((items) =>
            [
              `${source.data.title} and ${target.data.title} are already connected.`,
              ...items,
            ].slice(0, 30),
          );
          return;
    ...
    const resolveParameterizedVoiceAction = useCallback(
      ...
      [connectNodesFromVoice, promptAgentFromVoice],
    );
    ...
    const fileTargetsSource = useMemo<readonly FileTargetEntry[]>(
      () =>
        nodes
          .filter(
            (node) => node.data.kind === "file" && node.data.file !== undefined,
          )
          .map((node) => ({
            nodeId: node.id,
            title: node.data.title,
    ...
    const canvasImageNodesSource = useMemo<readonly CanvasImageNodeEntry[]>(
      () =>
        nodes
          .filter(
            (node) =>
              node.data.kind === "file" && node.data.file?.kind === "image",
          )
          .map((node) => {
    ...
    const agentSessionValue = useMemo<AgentSessionContextValue>(
      () => ({
        ...
        nodeTitle: (nodeId: string) =>
          nodesRef.current.find((node) => node.id === nodeId)?.data.title ??
          null,
    ...
    const workflowRuntimeValue = useMemo<WorkflowRuntimeContextValue>(
      () => ({
        ...
        busyAction: workflowBusyAction,
        mutationsAuthorized: workflowMutationsAuthorized,
        reviewGateFor: (nodeId: string) =>
          workflowReviewGates.get(nodeId) ?? null,
    ...
    const nodeCommentsValue = useMemo<NodeCommentsContextValue>(
      () => ({
        localCommentsFor: (nodeId) =>
          localCommentsForNode(pendingCanvas, nodeId),
        sharedCommentsFor: (nodeId) =>
          collaborationCommentsForNode(pendingCanvas, nodeId),
        ...
        createLocalComment: (nodeId, body) => {
          ...
          setCanvas(next);
          setEvents((items) =>
            ["Saved a private comment on this computer.", ...items].slice(
              0,
              80,
            ),
          );
    ...
    return (
      <main className="workspace-shell">
        <WorkspaceCommandBar
          ...
          workflowStatus={
            workflows.activeExecution?.status ??
            workflows.currentExecution?.status ??
            null
          }
        ...
        >
              ...
              <NodeCommentsProvider value={nodeCommentsValue}>
                <WorkspaceCanvas
                  ...
                  onEdgesChange={(changes: EdgeChange<WorkshopEdge>[]) => {
                    ...
                    const allowedChanges = filterLockedEdgeChanges(
                      changes,
                      edges,
                      nodes,
                    );
                    ...
                    if (
                      allowedChanges.some((change) => change.type === "remove")
                    )
                      record();
                    if (blockedRemoval) {
                      setEvents((items) =>
                    ...
                    setEdges((items) =>
                      applyEdgeChanges(
                        filterLockedEdgeChanges(changes, items, nodes),
                        items,
                      ),
                    );
                  }}
                  onConnect={(connection: Connection) => {
                    ...
                    if (!canConnectUnlocked(connection, nodes)) {
                      setEvents((items) =>
                        [
                          "Unlock both nodes before changing their connections.",
                          ...items,
                        ].slice(0, 30),
                      );
                      return;
                    ...
                    setEvents((items) =>
                      [
                        "Connected the nodes with a context link.",
                        ...items,
                      ].slice(0, 30),
                    );
                  }}
                  ...
                  collaborationGraphReadOnly={collaborationCanvas.graphReadOnly}
                  inheritedLockedNodeIds={inheritedContextLockIds(
                    nodes,
                    protectedNodeIds,
                  )}
          ...
          <WorkspaceActivityDrawer
            ...
            onCancelWorkflow={(executionId) =>
              void workflows.cancel(executionId)
            }
