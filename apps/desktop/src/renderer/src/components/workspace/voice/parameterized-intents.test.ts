...
describe('parameterized voice intents', () => {
  ...
  it('connects two exact live node titles and runs without confirmation', () => {
    ...
    const match = matchParameterizedVoiceIntent({
      ...
      nodes: [node('agent-1', 'Hermes', 'agent'), node('video-1', 'Walkthrough', 'video')],
      promptAgent: vi.fn(),
      connectNodes,
    });
  ...
  it('preserves free-form prompt casing and punctuation', () => {
    ...
    match?.action.run();
    expect(promptAgent).toHaveBeenCalledWith('agent-1', 'Fix APIClient.ts, then run Vitest.');
  });
  ...
  it('rejects ambiguous duplicate names and unrelated instructions', () => {
    ...
    expect(
      matchParameterizedVoiceIntent({
        ...
        nodes: [node('agent-1', 'Hermes', 'agent'), node('agent-2', 'Hermes', 'agent')],
      }),
    ).toBeNull();
    expect(
      matchParameterizedVoiceIntent({
...
function node(id: string, title: string, kind: 'agent' | 'video'): WorkshopNode {
  return {
    id,
    type: 'workshop',
    position: { x: 0, y: 0 },
