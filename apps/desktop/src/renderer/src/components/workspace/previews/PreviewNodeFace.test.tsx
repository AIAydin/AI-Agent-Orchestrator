...
const previewsOnEvent = vi.fn<
  (listener: (event: unknown) => void) => () => void
>(() => () => undefined);
const previewsSetAllowedOrigin = vi.fn(() =>
  Promise.resolve({ ok: true, value: null }),
);
const chromeStatus = vi.fn<
  () => Promise<{ ok: true; value: BrowserCompanionStatus }>
>(() =>
  Promise.resolve({
    ok: true,
    value: {
...
const chromeSetViewport = vi.fn(() =>
  Promise.resolve({ ok: true, value: null }),
);
const chromeDispatchInput = vi.fn(() =>
  Promise.resolve({ ok: true, value: null }),
);
...
beforeEach(() => {
  ...
  chromeSnapshot.mockImplementation(() =>
    Promise.resolve({ ok: true, value: null }),
  );
  chromeFrame.mockImplementation(() =>
    Promise.resolve({ ok: true, value: null }),
  );
...
describe("PreviewNodeFace", () => {
  ...
  it("renders a partitioned localhost webview once a port is set", () => {
    ...
    expect(webview?.closest(".preview-face-body")?.className).toContain(
      "nowheel",
    );
    expect(webview?.closest(".preview-face-body")?.className).toContain(
      "nodrag",
  ...
  it("toggles orientation and side-by-side comparison from the config popover", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Configure preview" }));
    fireEvent.click(
      screen.getByRole("button", { name: "Rotate to landscape" }),
    );
    expect(updateNodeData).toHaveBeenCalledWith("n1", {
  ...
  it("keeps Chrome sign-in separate from explicit connected-agent page access", () => {
    ...
    fireEvent.click(
      screen.getByLabelText("Let connected agents read this page"),
    );
    expect(updateNodeData).toHaveBeenCalledWith("n1", {
  ...
  it("opens an external website in the managed Chrome companion and never mounts it in Electron", async () => {
    ...
    expect(container.querySelector("webview")).toBeNull();
    fireEvent.click(
      screen.getByRole("button", { name: "Open in Google Chrome" }),
    );
    await waitFor(() =>
  ...
  describe("URL mode", () => {
    ...
    it("leaves browser navigation controls in Chrome rather than forwarding them through Electron", () => {
      ...
      expect(screen.getByRole("button", { name: "Go back" })).toHaveProperty(
        "disabled",
        true,
      );
    ...
    it("hides the dev-server Start/Stop control in URL mode", () => {
      renderFace("web-preview", { url: "https://app.staging.com/" });
      expect(
        screen.queryByRole("button", { name: "Start dev server" }),
      ).toBeNull();
      expect(
        screen.queryByRole("button", { name: "Stop dev server" }),
      ).toBeNull();
      expect(
        screen.getByRole("button", { name: "Open in Google Chrome" }),
      ).toBeTruthy();
        ...
        >
          <AgentSessionProvider value={sessionValue()}>
            <PreviewNodeFace
              id="n1"
              kind="web-preview"
              data={nodeData({ previewPort: 5173 })}
            />
          </AgentSessionProvider>
        </CanvasNodeInteractionProvider>,
      );
