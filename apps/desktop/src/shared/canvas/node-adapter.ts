...
export function canonicalNodeFromLegacy(
  ...
  const parsed = CanvasNodeSchema.safeParse({
    ...
    type,
    title:
      stringValue(node.data["title"]) ??
      matchingPrevious?.title ??
      titleFor(type),
    color:
      colorValue(node.data["color"]) ?? matchingPrevious?.color ?? "#82909b",
    icon:
      stringValue(node.data["icon"]) ??
      matchingPrevious?.icon ??
      LEGACY_KINDS[type],
    ...
    size: {
      width: Math.max(
        ...
        positiveNumber(node.width) ??
          matchingPrevious?.size.width ??
          fallbackDimensions.width,
      ),
      height: Math.max(
        minimumDimensions.height,
        positiveNumber(node.height) ??
          matchingPrevious?.size.height ??
          fallbackDimensions.height,
      ),
    },
    collapsed:
      booleanValue(node.data["collapsed"]) ??
      matchingPrevious?.collapsed ??
      false,
    locked:
      booleanValue(node.data["locked"]) ?? matchingPrevious?.locked ?? false,
    ...(matchingPrevious?.groupId === undefined
      ? {}
      : { groupId: matchingPrevious.groupId }),
    comments: matchingPrevious?.comments ?? [],
    status: legacyStatus(node.data["canonicalStatus"] ?? node.data["status"]),
    runHistoryIds: mergeRunHistory(
      matchingPrevious?.runHistoryIds ?? [],
      node.data["runId"],
    ),
  ...
  return {
    issue: {
      code:
        type === "extension" ? "INVALID_EXTENSION_NODE" : "INVALID_TYPED_NODE",
      entityId: node.id,
...
function canonicalData(
  ...
  switch (type) {
    ...
    case "video":
      return compact({ ...base, file: raw["file"] });
    case "diff-review":
      return compact({
        ...base,
        ...
        showWhitespace: booleanValue(
          rendererValueOrPrevious(raw, previous, "showWhitespace"),
        ),
        ignoreWhitespace: booleanValue(raw["ignoreWhitespace"]),
        hunkDecisions: raw["hunkDecisions"],
        lineCommentIds: stringArray(
          rendererValueOrPrevious(raw, previous, "lineCommentIds"),
        ),
        revisionRequest:
          typeof raw["revisionRequest"] === "string"
            ? raw["revisionRequest"]
            : undefined,
    ...
    case "git-pr":
      return compact({
        ...base,
        deliveryTarget: rendererValueOrPrevious(
          raw,
          previous,
          "deliveryTarget",
        ),
        ...
        worktreeId: stringValue(
          rendererValueOrPrevious(raw, previous, "worktreeId"),
        ),
        ...
        destinationBranch: stringValue(
          rendererValueOrPrevious(raw, previous, "destinationBranch"),
        ),
        baseBranch: stringValue(
          rendererValueOrPrevious(raw, previous, "baseBranch"),
        ),
        pullRequestTitle: stringValue(
          rendererValueOrPrevious(raw, previous, "pullRequestTitle"),
        ),
        pullRequestBody: stringRendererValueOrPrevious(
          raw,
          previous,
          "pullRequestBody",
        ),
        pullRequestDraft: booleanValue(
          rendererValueOrPrevious(raw, previous, "pullRequestDraft"),
        ),
        commitIds: stringArray(
          rendererValueOrPrevious(raw, previous, "commitIds"),
        ),
        ahead: rendererValueOrPrevious(raw, previous, "ahead"),
        behind: rendererValueOrPrevious(raw, previous, "behind"),
        mergeReadiness: rendererValueOrPrevious(
          raw,
          previous,
          "mergeReadiness",
        ),
        pullRequestUrl: stringValue(
          rendererValueOrPrevious(raw, previous, "pullRequestUrl"),
        ),
        checkIds: stringArray(
          rendererValueOrPrevious(raw, previous, "checkIds"),
        ),
      });
    case "diagram":
      return compact({
    ...
    case "group-frame":
      return compact({
        ...base,
        purpose:
          raw["purpose"] ?? (previous === undefined ? "custom" : undefined),
...
function canonicalPreviewConfiguration(
  ...
  const previewCommand = previewValueOrPrevious(
    raw,
    previous,
    "previewCommand",
    "command",
  ...
  return compact({
    ...
    urlPath: stringValue(
      previewValueOrPrevious(raw, previous, "previewUrlPath", "urlPath"),
    ),
    ...
    orientation: previewValueOrPrevious(
      raw,
      previous,
      "previewOrientation",
      "orientation",
    ...
    browserAuthenticationEnabled: booleanValue(
      previewValueOrPrevious(
        raw,
        previous,
    ...
    agentBrowserAccess: booleanValue(
      previewValueOrPrevious(
        raw,
        previous,
        "agentBrowserAccess",
    ...
    comparison: previewValueOrPrevious(
      raw,
      previous,
      "previewComparison",
      "comparison",
...
function previewValueOrPrevious(
  ...
  if (Object.prototype.hasOwnProperty.call(raw, rendererKey))
    return raw[rendererKey];
