...
export const CollaborationCanvasMetadataSchema = z
  ...
  .strict();
export type CollaborationCanvasMetadata = z.infer<
  typeof CollaborationCanvasMetadataSchema
>;
...
export type CollaborationFileAvailability = z.infer<
  typeof CollaborationFileAvailabilitySchema
>;
...
export const CollaborationNodeMetadataSchema = z
  ...
  .superRefine((node, context) => {
    const hasFileMetadata =
      node.localResourceId !== undefined || node.availability !== undefined;
    if (node.type !== "file" && hasFileMetadata) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["availability"],
        message:
          "Only file nodes can expose local resource availability metadata.",
      });
    }
...
export type CollaborationNodeMetadata = z.infer<
  typeof CollaborationNodeMetadataSchema
>;
...
export const CollaborationEdgeMetadataSchema = z
  ...
  .strict();
export type CollaborationEdgeMetadata = z.infer<
  typeof CollaborationEdgeMetadataSchema
>;
...
export type CollaborationGroupPurpose = z.infer<
  typeof CollaborationGroupPurposeSchema
>;
...
export type CollaborationGroupLayout = z.infer<
  typeof CollaborationGroupLayoutSchema
>;
...
export const CollaborationGroupMetadataSchema = z
  ...
  .strict();
export type CollaborationGroupMetadata = z.infer<
  typeof CollaborationGroupMetadataSchema
>;
...
export const CollaborationTaskMetadataSchema = z
  ...
  .strict();
export type CollaborationTaskMetadata = z.infer<
  typeof CollaborationTaskMetadataSchema
>;
...
export const CollaborationCommentMetadataSchema = z
  ...
  .strict();
export type CollaborationCommentMetadata = z.infer<
  typeof CollaborationCommentMetadataSchema
>;
...
export const CollaborationWorkflowMetadataSchema = z
  ...
  .strict();
export type CollaborationWorkflowMetadata = z.infer<
  typeof CollaborationWorkflowMetadataSchema
>;
...
export const CollaborationReviewMetadataSchema = z
  ...
  .strict();
export type CollaborationReviewMetadata = z.infer<
  typeof CollaborationReviewMetadataSchema
>;

function boundedRecord<Schema extends z.ZodTypeAny>(
  schema: Schema,
  maximum: number,
) {
  ...
  CollaborationMetadataSnapshotBaseSchema.superRefine((snapshot, context) => {
    ...
    for (const [nodeId, node] of Object.entries(snapshot.nodes)) {
      if (
        ...
        snapshot.groups[node.groupId] === undefined
      ) {
        addReferenceIssue(
          context,
          ["nodes", nodeId, "groupId"],
          "Node group does not exist.",
        );
      }
      if (
        node.taskId !== undefined &&
        snapshot.tasks[node.taskId] === undefined
      ) {
        addReferenceIssue(
          context,
          ["nodes", nodeId, "taskId"],
    ...
    for (const [taskId, task] of Object.entries(snapshot.tasks)) {
      if (snapshot.nodes[taskId]?.type !== "task") {
        addReferenceIssue(
          context,
          ["tasks", taskId],
          "Task metadata must belong to a task node.",
        );
      }
    ...
    for (const [commentId, comment] of Object.entries(snapshot.comments)) {
      if (
        ...
        snapshot.nodes[comment.nodeId] === undefined
      ) {
        addReferenceIssue(
          context,
      ...
      if (
        comment.taskId !== undefined &&
        snapshot.tasks[comment.taskId] === undefined
      ) {
        addReferenceIssue(
          context,
      ...
      if (
        ...
        snapshot.comments[comment.replyToId] === undefined
      ) {
        addReferenceIssue(
          context,
    ...
    for (const [workflowId, workflow] of Object.entries(snapshot.workflow)) {
      if (
        ...
        snapshot.nodes[workflow.nodeId] === undefined
      ) {
        addReferenceIssue(
          context,
    ...
    for (const [reviewId, review] of Object.entries(snapshot.reviews)) {
      if (snapshot.nodes[review.nodeId] === undefined) {
        addReferenceIssue(
          context,
          ["reviews", reviewId, "nodeId"],
          "Review node does not exist.",
        );
      }
...
export type CollaborationMetadataSnapshot = z.infer<
  typeof CollaborationMetadataSnapshotSchema
>;
...
function assertRecordIdentities(
  ...
  for (const [recordId, record] of Object.entries(records)) {
    ...
    addReferenceIssue(
      context,
      [...path, recordId, "id"],
      "Record key must match its identifier.",
    );
  }
}
