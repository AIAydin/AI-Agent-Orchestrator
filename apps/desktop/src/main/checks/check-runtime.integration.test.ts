...
import type { CheckEventEnvelope, CheckExecutionView } from '../../shared/checks/contracts.js';
import {
  AppSettingsSchema,
  type AppSettings,
  type Project,
...
describe('CheckRuntime', () => {
  ...
  it('checks final authorization after async drift resolution and before creating a process', async () => {
    const fixture = await createFixture();
    const marker = join(fixture.projectRoot, 'revoked-check-must-not-run');
    fixture.settings.current = settingsFor(fixture, {
      lintCommand: {
    ...
    const { runtime } = runtimeFor(fixture);
    const plan = await runtime.prepare(71, { projectId: PROJECT_ID, checkId: 'lint' });

    await expect(
      runtime.start(71, plan.planId, () => {
        throw new Error('The remembered approval was revoked during revalidation.');
      }),
    ).rejects.toThrow('revoked during revalidation');
    await expect(access(marker)).rejects.toThrow();
    expect(fixture.store.executions.size).toBe(0);
    expect(fixture.store.audits.at(-1)).toEqual({
      category: 'check',
      action: 'launch',
      outcome: 'denied',
    });
