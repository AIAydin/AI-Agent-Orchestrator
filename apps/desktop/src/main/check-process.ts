...
async function fileIdentity(path: string): Promise<FileIdentity> {
  ...
  if (!details.isFile()) throw new Error('The configured check executable is not a file.');
  return identity(path, details);
}
...
async function packageScriptIdentity(
  ...
  if (!details.isFile() || details.size > MAX_PACKAGE_JSON_BYTES) {
    throw new Error('The project package.json is not an ordinary bounded file.');
  }
  const contents = await readFile(canonical);
  if (contents.byteLength !== details.size || contents.byteLength > MAX_PACKAGE_JSON_BYTES) {
    throw new Error('The project package.json changed while its check plan was prepared.');
  }
  return [
    {
      ...identity(canonical, details),
