...
class FakeWebRequest {
  ...
  onBeforeRequest(
    _filter: { urls: string[] },
    listener: FakeWebRequest["listener"],
  ): void {
...
describe("installPreviewWebviewSecurity", () => {
  ...
  it("denies window.open and hands confirmed http(s) links to the system browser", async () => {
    ...
    expect(options.confirmOpenExternal).toHaveBeenCalledWith(
      "https://example.com/docs",
    );
  });
  ...
  it("URL mode opens popup-style HTTP links in the same hardened preview tab", async () => {
    ...
    expect(
      contents.windowOpenHandler?.({ url: "https://www.granola.ai/" }),
    ).toEqual({
      action: "deny",
    });
    await vi.waitFor(() =>
  ...
  it("authentication mode allows a hardened OAuth window in the same persistent session", () => {
    ...
    expect(options.openExternal).not.toHaveBeenCalled();
    expect(onGuestCreated).toHaveBeenCalledWith(
      "persist:preview:p1:n1",
      contents,
    );
  });
  ...
  it("filters guest network requests to loopback, mapping websocket schemes", () => {
    ...
    const { session } = attach();
    expect(
      session.webRequest.allows("http://127.0.0.1:5173/assets/app.js"),
    ).toBe(true);
    expect(session.webRequest.allows("ws://localhost:5173/hmr")).toBe(true);
    expect(session.webRequest.allows("https://example.com/analytics.js")).toBe(
      false,
    );
  });
  ...
  it("uses the captured session when a late request arrives after the guest is destroyed", () => {
    ...
    expect(() =>
      browserSession.webRequest.allows("https://miro.com/app"),
    ).not.toThrow();
  ...
  it("URL mode: permits safe cross-origin navigation before and after the first page commits", () => {
    ...
    const { attach } = createHarness({
      allowedOriginForGuestSession: (session) =>
        (session as unknown) === (targetSession as unknown)
    ...
    contents.emit(
      "will-navigate",
      samePage,
      "https://app.staging.com/other-page",
    );
  ...
  it("URL mode: the guest session request filter allows a cross-origin subresource that loopback mode would cancel", () => {
    ...
    const { attach } = createHarness({
      allowedOriginForGuestSession: (session) =>
        (session as unknown) === (externalSession as unknown)
          ? "https://app.staging.com"
          : null,
    });
    ...
    expect(
      loopbackSession.webRequest.allows("https://cdn.example.com/font.woff2"),
    ).toBe(false);
    expect(
      externalSession.webRequest.allows("https://cdn.example.com/font.woff2"),
    ).toBe(true);
    // Credentialed URLs are still rejected in URL mode.
    expect(
      externalSession.webRequest.allows("https://user:pass@cdn.example.com/x"),
    ).toBe(false);
  });
...
describe("shouldAttachPreviewWebview", () => {
  it("requires a preview partition, no preload, and a loopback (or blank) src", () => {
    expect(
      shouldAttachPreviewWebview({ partition: "preview:p1:n1", src: ALLOWED }),
    ).toBe(true);
    expect(shouldAttachPreviewWebview({ partition: "preview:p1:n1" })).toBe(
      true,
    );
    expect(
      shouldAttachPreviewWebview({
    ...
    expect(
      shouldAttachPreviewWebview({ partition: "other", src: ALLOWED }),
    ).toBe(false);
...
describe("url helpers", () => {
  it("allowedGuestNavigation admits blank/data before pin and enforces the pin after", () => {
    ...
    expect(allowedGuestNavigation("http://localhost:5173/x", pinned)).toBe(
      true,
    );
    expect(allowedGuestNavigation("http://localhost:9999/", pinned)).toBe(
      false,
    );
  });
  ...
  it("allowedGuestNavigation uses the configured origin as browser-mode authority", () => {
    ...
    expect(
      allowedGuestNavigation(ALLOWED, null, "https://app.staging.com"),
    ).toBe(true);
  });

  it("isAllowedGuestRequest rejects credentials and non-loopback hosts", () => {
    expect(isAllowedGuestRequest("http://user:pass@127.0.0.1:5173/")).toBe(
      false,
    );
  ...
  it("isAllowedGuestRequest: URL mode allows any http(s)/ws(s) subresource (no credentials) that loopback mode rejects", () => {
    ...
    expect(
      isAllowedGuestRequest("file:///etc/passwd", "https://app.staging.com"),
    ).toBe(false);
    // Loopback mode (no configured origin) is unaffected — byte-for-byte unchanged.
    expect(isAllowedGuestRequest("https://cdn.example.com/font.woff2")).toBe(
      false,
    );
  });
});
