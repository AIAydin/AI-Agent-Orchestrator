...
export function createDefaultSettings(): AppSettings {
  ...
  return {
    ...
    worktreeRoot: join(
      documents,
      PRODUCT.dataDirectoryName,
      PRODUCT.worktreeDirectoryName,
    ),
...
export function registerIpcHandlers(
  ...
  const withProjectGitAuthorization = async <Output>(
    ...
    const value = await repositories.git.withDelegateAuthorization(
      createNativeGitDelegateAuthorizer({
        ...
        show: async (options) =>
          (await dialog.showMessageBox(authority.parent, options)).response,
      }),
  ...
  const diagramExports = new DiagramExportService(
    ...
    (action, outcome, metadata) =>
      store.appendAudit("export", action, outcome, metadata),
  );
  ...
  const updates = new UpdateIpcService(
    dialog,
    shell,
    store,
    () => app.getVersion(),
  ...
  const voice = new VoiceIpcService(
    dialog,
    store,
  ...
  const projectClones = new ProjectCloneIpcService(
    dialog,
    projects,
    outbound,
    runDataOperation,
  ...
  const agentPeersStore: AgentPeersStore = {
    loadCanvas: (projectId) => {
      ...
      if (document === undefined || document.canonical === undefined)
        return null;
      return {
  ...
  const readiness = new AgentReadinessIpcService(
    dialog,
    agentReadiness,
    store,
    runDataOperation,
  ...
  const commandReadiness = new CommandReadinessService(
    store,
    app.getPath("home"),
  );
  ...
  const unsubscribeBackupChanges = store.subscribeToDurableChanges(() =>
  ...
  const settings = new SettingsIpcService(
    ...
    async (current, next) =>
      await settingsPersistenceReadiness.verify(current, next),
    () => backups.refreshSchedule(),
    runDataOperation,
  );
  const folderReadiness = new FolderReadinessIpcService(
    folderReadinessService,
    runDataOperation,
  );
  const runs = new RunService(
    ...
    () => store.getSettings(createDefaultSettings()),
    async (adapterId) =>
      (await extensions.listActiveAgentAdapters()).find(
        (adapter) => adapter.id === adapterId,
      ),
    async (adapterId, expectedManifest, launch) =>
      await extensions.launchTrustedAdapter(
        adapterId,
        expectedManifest,
        launch,
      ),
  ...
  const browserCompanion = new BrowserCompanionIpcService(
    new BrowserCompanionService({
      ...
      previewBrowser:
        previewBrowser instanceof PreviewAgentBrowser
          ? previewBrowser
          : new PreviewAgentBrowser(),
  ...
  const workflowArtifactResolver = new WorkflowArtifactActionResolver(
    store,
    new GitTargetResolver(store, repositories, () =>
      store.getSettings(createDefaultSettings()),
    ),
    join(app.getPath("userData"), "verified-test-artifacts"),
  );
  const workflows = new WorkflowIpcService(
    dialog,
    store,
    workflowRuntime.createHost,
    {
      ...
      authorizeMutation: (event) =>
        collaboration.assertWorkflowMutationAuthorized(event.sender),
      resolveArtifact: async (input, action) =>
        await workflowArtifactResolver.resolve(input, action),
      nativeShell: shell,
    },
  );
  ...
  const deliveryExactResolver = new ExactCheckResolver(
    store,
    deliveryTargets,
    () => store.getSettings(createDefaultSettings()),
  );
  const deliveryExactExecutor = new ExactCheckExecutor(
    store,
    deliveryTargets,
    () => store.getSettings(createDefaultSettings()),
  );
  ...
  const deliveryReadiness = new GitDeliveryReadinessIpcService(
    dialog,
    deliveryAuthority,
    store,
  );
  const shippingReadiness = new DeliveryReadinessShippingAuthority(
    deliveryAuthority,
  );
  ...
  const gitRemote = new GitRemoteDeliveryIpcService(
    ...
    new GitRemoteDeliveryService(
      ...
      {
        defaultRemote: () =>
          store.getSettings(createDefaultSettings()).gitRemote,
        githubCliRuntime: githubCli,
      },
    ),
    store,
  );
  const gitConnectionsMutations = new GitConnectionsMutationCoordinator(
    gitRemote,
  );
  const gitConnections = new GitConnectionsIpcService(
    dialog,
    new GitConnectionsService(
      store,
      new GitRemoteConfigurationService(repositories),
      {
        withMutationAdmission: async (operation) =>
          await gitConnectionsMutations.withRemoteConfigurationMutation(
            operation,
          ),
      },
    ),
    ...
    undefined,
    async (operation) =>
      await gitConnectionsMutations.withGitHubCliMutation(operation),
  );
  ...
  const pauseForShutdown = async (includeRecovery = true): Promise<void> => {
    ...
    if (includeRecovery)
      operations.push(recovery.pauseForExternalDataMutation());
    await awaitDataServices(operations);
    shutdownServicesPaused = true;
  };
  const pauseForDataImport = async (
    mode: "merge" | "replace",
  ): Promise<void> => {
  ...
  const startupRetention = store.applyRetention(
    store.getSettings(createDefaultSettings()),
  );
  ...
  handleWithEvent(
    ...
    async (event, input) =>
      await runDataOperation(async () => {
        const authority = requireIpcWindowAuthority(
          event,
        ...
        authority.assertCurrent();
        if (result.ready)
          commandReadiness.recordVerifiedSettingsReadiness(result);
        return result;
      }),
  );
  handleWithEvent(
  ...
  handle(
    ...
    async (input) =>
      await runDataOperation(() =>
        ApprovalViewSchema.array().parse(approvals.list(input)),
      ),
  );
  handleWithEvent(
    ...
    async (event, input) =>
      await runDataOperation(() => {
        requireIpcWindowAuthority(
          event,
          "Saved approval revocation",
        ).assertCurrent();
  ...
  handleWithEvent(
    ...
    async (event) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(() =>
            projects.refreshRecentProjects(authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event, projectId) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.refreshProject(projectId, authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.pickRepository(authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.pickParent(authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.pickExecutable(authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.pickExternalApplication(authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event, input) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.pickReferences(input, authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event, input) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () =>
              await projects.selectMovedProject(input.projectId, authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event, input) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.confirmMovedProject(input, authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event, path) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.open(path, authority),
          ),
      ),
  );
  handleWithEvent(
  ...
  handleWithEvent(
    ...
    async (event) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.createDemo(authority),
          ),
      ),
  );
  handleWithEvent(
    ...
    async (event, projectId) =>
      await withProjectGitAuthorization(
        ...
        async (authority) =>
          await runDataOperation(
            async () => await projects.initializeGit(projectId, authority),
          ),
      ),
  );
    ...
    async (event, projectId) =>
      await runDataOperation(() => {
        ...
        if (
          !store
            .listProjects()
            .some((project) => project.id === projectId && !project.missing)
        ) {
      ...
      await runDataOperation(() => {
        ...
        if (
          !store
            ...
            .some(
              (project) =>
                project.id === document.projectId && !project.missing,
            )
        ) {
          throw new Error("The selected project is no longer available.");
        }
    ...
    async (event, projectId) =>
      await runDataOperation(() => {
        ...
        return {
          document,
          history:
            store.loadCanvasHistory(projectId) ??
            emptyCanvasHistory(projectId, document.id),
        };
      }),
  );
      ...
      await runDataOperation(() => {
        ...
        if (
          !store
            ...
            .some(
              (project) =>
                project.id === input.document.projectId && !project.missing,
            )
        ) {
          throw new Error("The selected project is no longer available.");
        }
  ...
  handle(
    ...
    async (input) =>
      await runDataOperation(() => store.listAuditEvents(input.limit)),
  );
        ...
        return await performAuditedLocalEffect({
          ...
          effect: async () => {
            ...
            if (outcome.status !== "created") {
              throw new Error(
                "Forgeboard could not create the requested backup.",
              );
            }
  ...
  handle(IPC_CHANNELS.storageBackupHealth, z.tuple([]), async () =>
    BackupHealthSchema.parse(
      await runDataOperation(() => store.getBackupHealth()),
    ),
  );
  handle(
  ...
  handleWithEvent(
    ...
    async (event, confirmation) => {
      ...
      const confirmed = await confirmPrivacyDeletion(
        dialog,
        authority.parent,
        () => {
          authority.assertCurrent();
      ...
      try {
        ...
        return await performPrivacyDeletion({
          ...
          listMissingBackupIds: async () =>
            await store.listMissingRecordedBackupIds(),
          confirmForgetMissingBackups: async (count) => {
            ...
            const decision = await dialog.showMessageBox(authority.parent, {
              ...
              buttons: [
                "Cancel deletion",
                "Forget missing backups and continue",
              ],
              defaultId: 0,
              cancelId: 0,
              noLink: true,
          ...
          authorizeDeletion: () => {
            ...
            store.appendAudit("privacy", "delete-all-local-data", "allowed", {
              effect:
                "erase-all-forgeboard-local-data-including-this-audit-event",
            });
          },
  ...
  return {
    ...
    prepareToQuit: async () => {
      await prepareReversibleQuitBackup({
        ...
        prepareBackup: async () => {
          ...
          return outcome.status === "missing-destination"
            ? "missing-destination"
            : "ready";
        },
    ...
    dispose: async () => {
      ...
      if (
        dataOperations.mutationKind !== null &&
        dataOperations.mutationKind !== "quit"
      ) {
...
function handle<Args extends unknown[], Output>(
  ...
  ipcMain.handle(
    channel,
    async (event, ...rawArgs: unknown[]): Promise<IpcResult<Output>> => {
      try {
        const authority = requireIpcWindowAuthority(
          event,
          "Forgeboard request",
...
function gitHubCliValidationEnvironment(): Readonly<
