...
export async function checkDockerReadiness(
  ...
  const containerName = `forgeboard-readiness-${randomUUID()}`;
  ...
  if (probe.timedOut) await removeProbeContainer(executable, containerName);
...
async function removeProbeContainer(executable: string, name: string): Promise<void> {
  await runBoundedCommand(executable, ['rm', '--force', name], 2_000, 2_048).catch(() => undefined);
}
