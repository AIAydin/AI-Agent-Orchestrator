...
describe('immutable agent context snapshots', () => {
  ...
  it('materializes source-free generated context into the same revalidated private snapshot', async () => {
    const checkout = await temporaryRoot('forgeboard-generated-context-');
    const logicalPath = path.join(checkout, '.forgeboard-context', 'brief-abc.md');
    const content = '# Reviewed brief\n\nExact generated bytes.\n';
    ...
    const context: AgentExecutionContextRequest = {
      ...
      generatedArtifacts: [{ path: logicalPath, content, sha256: digest }],
      manifestId: 'generated-context-test',
      manifestDigest: 'b'.repeat(64),
    };
    const fixture: HostFixture = { checkout, sourcePath: logicalPath, context };
    const plan = hostPlan(fixture, {
      prompt: `Inspect ${logicalPath}`,
      strategy: 'prompt-references',
      transport: 'argument',
    });

    await expect(access(logicalPath)).rejects.toThrow();
    const snapshot = await createImmutableContextSnapshot(context, checkout, { runtime: 'host' });
    const privatePath = snapshot!.bindings[0]!.snapshotPath;
    expect(await readFile(privatePath, 'utf8')).toBe(content);
    const rebound = await snapshot!.bind(plan);
    expect(contextLaunchFields(rebound).join('\n')).toContain(privatePath);
    expect(contextLaunchFields(rebound).join('\n')).not.toContain(logicalPath);
    await snapshot!.cleanup();
    await expect(access(snapshot!.rootPath)).rejects.toThrow();
  });
  ...
  it('adds one read-only Docker context-root mount with deterministic container paths', async () => {
    ...
    const sourcePath = path.join(checkout, '.forgeboard-context', 'brief.md');
    const generatedContent = 'docker generated context\n';
    const generatedDigest = sha256(generatedContent);
    const context: AgentExecutionContextRequest = {
      ...contextFor([[sourcePath, generatedDigest]]),
      generatedArtifacts: [
        { path: sourcePath, content: generatedContent, sha256: generatedDigest },
      ],
    };
    ...
    const dockerPlan = await planDockerAgentLaunch(providerPlan, {
      ...
      virtualContextAttachments: context.attachments,
    });
    const snapshot = await createImmutableContextSnapshot(context, checkout, {
      runtime: 'docker',
      managedRoot,
    ...
    expect(rebound.disclosure.contextAttachments[0]?.path).toBe('/forgeboard-context/0000.md');
    expect(rebound.disclosure.permissionProfile.readRoots).toContain('/forgeboard-context');
    expect(contextLaunchFields(rebound).join('\n')).toContain('file:///forgeboard-context/0000.md');
    expect(contextLaunchFields(rebound).join('\n')).not.toContain(logicalFileUrl);
    expect(contextLaunchFields(rebound).join('\n')).not.toContain(sourcePath);
    expect(contextLaunchFields(rebound).join('\n')).not.toContain(
      '/workspace/.forgeboard-context/brief.md',
    );
    await snapshot!.cleanup();
  });
