...
import { assertCompletionEvidenceIdentity } from './workflow-completion-evidence.js';
import {
...
import {
  completeInternalReviewGate,
  queueEligibleRevisionAttempts,
} from './workflow-review-gate.js';
...
export class WorkflowHost {
  ...
  async #pumpLocked(
    ...
    for (let iteration = 0; iteration < maximumIterations; iteration += 1) {
      ...
      for (const nodeId of scheduling.runnableNodeIds) {
        if (bindings.has(nodeId)) continue;
  ...
  #watchCompletion(
    ...
    void handle.completion
      ...
      .catch((error: unknown) => {
        ...
        this.#notify(executionId, 'host-error', occurred, { message: errorMessage(error) }, nodeId);
      });
  }
  ...
  #state(executionId: string, existing?: WorkflowExecutionRecord): WorkflowHostState {
    ...
    const approvals = this.store
      .listWorkflowNodeBindings(executionId)
      .map(parseBinding)
      .flatMap((binding): WorkflowApprovalRequestView[] => {
        if (
          binding.phase !== 'waiting-approval' ||
    ...
    return { execution, runtime, scheduling: getSchedulingSnapshot(runtime), approvals };
  }
