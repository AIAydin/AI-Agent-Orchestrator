...
import { ipcResultSchema, type IpcResult } from '../../shared/contracts.js';
import {
...
import type { WorkflowHost, WorkflowHostState } from './workflow-host.js';
import type {
  WorkflowHostInteractionNotification,
  WorkflowHostNotification,
} from './workflow-host-contracts.js';
import { workflowHostStateToView } from './workflow-view.js';
...
export class WorkflowIpcService {
  ...
  #onHostNotification(notification: WorkflowHostNotification): void {
    ...
    void this.#host
      ...
      .then((state) => {
        if (this.#disposed || this.#executionOwners.get(notification.executionId) !== ownerToken) {
          return;
        }
