...
describe("ProjectVideoService", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await realpath(
      await mkdtemp(path.join(tmpdir(), "forgeboard-videos-")),
    );
  ...
  afterEach(
    async () => await rm(fixtureRoot, { recursive: true, force: true }),
  );
  ...
  it("selects a signature-validated project video and issues no absolute renderer path", async () => {
    ...
    const video = service();
    await expect(
      video.choose({ projectId: PROJECT_ID }, vi.fn()),
    ...
    if (loaded.status !== "available")
      throw new Error("Expected playable video");
    await expect(
      video.fileUrlForProtocolRequest(loaded.playbackUrl),
    ).resolves.toMatch(/^file:/u);
  });
  ...
  it("imports a selected external video into the project without overwriting existing media", async () => {
    ...
    await writeFile(
      path.join(projectRoot, "forgeboard-videos", "demo.mp4"),
      MP4_BYTES,
    );
    showOpenDialog.mockResolvedValue({
      canceled: false,
    ...
    await expect(
      service().choose({ projectId: PROJECT_ID }, vi.fn()),
    ).resolves.toMatchObject({
      projectId: PROJECT_ID,
