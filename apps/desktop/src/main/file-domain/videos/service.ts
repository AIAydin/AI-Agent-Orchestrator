...
import { copyFile, lstat, mkdir, open, realpath, stat, unlink } from 'node:fs/promises';
...
export class ProjectVideoService {
  ...
  public async fileUrlForProtocolRequest(requestUrl: string): Promise<string | null> {
...
async function chooseOrImportProjectVideo(root: string, selectedPath: string): Promise<string> {
  try {
...
async function importExternalVideo(root: string, selectedPath: string): Promise<string> {
...
async function canonicalProjectVideoDirectory(root: string, relativePath: string): Promise<string> {
  ...
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new FileDomainError('SYMLINK_BLOCKED', 'The project video import folder is not safe.');
  }
...
async function inspectVideoFile(
  ...
  if (!metadata.isFile()) throw new FileDomainError('NOT_A_FILE', 'Choose an ordinary video file.');
  ...
  try {
    ...
    if (mimeType === null) {
      throw new FileDomainError('BINARY_FILE', 'Choose an MP4, WebM, or Ogg video file.');
    }
...
async function canonicalProjectVideo(
  ...
  const lexical = path
    .relative(root, allowAbsolute ? path.resolve(candidate) : path.resolve(root, candidate))
    .split(path.sep)
    .join('/');
  ...
  if (canonical.relativePath === '' || path.isAbsolute(canonical.relativePath)) {
    throw new FileDomainError('PATH_OUTSIDE_PROJECT', 'Choose a video inside this project.');
  }
  return canonical.relativePath;
}
...
function videoMimeType(
  ...
  if (['.mp4', '.m4v', '.mov'].includes(extension) && prefix.subarray(4, 8).toString() === 'ftyp') {
    return 'video/mp4';
  }
  if (
    extension === '.webm' &&
  ...
  if (['.ogv', '.ogg'].includes(extension) && prefix.subarray(0, 4).toString() === 'OggS') {
    return 'video/ogg';
  }
  return null;
}
