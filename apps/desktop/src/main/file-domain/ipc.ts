...
export class FileIpcService {
  ...
  public constructor(
    ...
    private readonly runOperation: FileOperationRunner = async (operation) =>
      await operation(),
  ...
  public registerIpcHandlers(): void {
    if (this.#registered)
      throw new Error("The file IPC handlers are already registered.");
    ...
    this.#handle(
      ...
      async (authority, rawArgs) => {
        ...
        authority.assertCurrent();
        const errorMessage = await this.nativeShell.openPath(
          target.absolutePath,
        );
        authority.assertCurrent();
        if (errorMessage !== "") {
    ...
    if (imageOperations !== undefined) {
      this.#handle(
        ...
        async (authority, rawArgs) => {
          const [input] = z
            .tuple([ProjectImageChooseInputSchema])
            .parse(rawArgs);
          const reference = await imageOperations.choose(input, () =>
            authority.assertCurrent(),
          );
          return ProjectImageReferenceSchema.nullable().parse(
            reference ?? null,
          );
        },
      );
      this.#handle(
        ...
        async (_authority, rawArgs) => {
          ...
          return ProjectImageLoadResultSchema.parse(
            await imageOperations.load(input),
          );
        },
      );
    }
    ...
    if (videoOperations !== undefined) {
      this.#handle(
        ...
        async (authority, rawArgs) => {
          const [input] = z
            .tuple([ProjectVideoChooseInputSchema])
            .parse(rawArgs);
          const reference = await videoOperations.choose(input, () =>
            authority.assertCurrent(),
          );
          return ProjectVideoReferenceSchema.nullable().parse(
            reference ?? null,
          );
        },
      );
      this.#handle(
        ...
        async (_authority, rawArgs) => {
          ...
          return ProjectVideoLoadResultSchema.parse(
            await videoOperations.load(input),
          );
        },
      );
    }
  ...
  public async videoFileUrlForRequest(
    requestUrl: string,
  ): Promise<string | null> {
  ...
  #handle<Output>(
    ...
    operation: (
      authority: FileInvocationAuthority,
      rawArgs: unknown[],
    ...
    ipcMain.handle(
      ...
      async (event, ...rawArgs: unknown[]): Promise<FileIpcResult<Output>> => {
        const pending = Promise.resolve().then(
          async () =>
            await this.#invoke(event, rawArgs, operationName, operation),
        );
  ...
  async #invoke<Output>(
    ...
    try {
      if (this.#disposed) {
        throw new FileDomainError(
          "IO_ERROR",
          "The local file service is unavailable.",
        );
      }
  ...
  const failure =
    cause instanceof FileDomainError
      ...
      : cause instanceof z.ZodError
        ? new FileDomainError(
            "INVALID_REQUEST",
            "The requested local file operation is invalid.",
          )
        : new FileDomainError(
            "IO_ERROR",
