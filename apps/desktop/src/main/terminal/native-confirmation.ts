...
export function terminalLaunchMessage(review: TerminalLaunchNativeReview): MessageBoxOptions {
  ...
  return {
    ...
    detail: [
      ...
      '',
      `This one-time launch approval expires at ${view.expiresAt}. Artemis rechecks the exact program identity, project, and folder immediately before starting it.`,
      'If you trust this exact launch for 30 days, Artemis will still recheck its program, arguments, project folder, and environment-variable names before every start. Any change requires another native approval.',
    ].join('\n'),
