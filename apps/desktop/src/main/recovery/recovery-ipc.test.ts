...
import { MAX_LOCAL_DATA_IMPORT_BYTES } from './import-file.js';
...
describe('RecoveryIpcService lifecycle', () => {
  ...
  it('rejects subframes at admission before reading or mutating recovery storage', async () => {
    const fixture = createStore();
    const harness = createHarness(fixture.store);
    const event = liveEvent(44);
    Object.defineProperty(event.value, 'senderFrame', { value: {} });
    ...
    expect(result).toMatchObject({ ok: false, error: { code: 'OPERATION_FAILED' } });
    expect(ipcErrorMessage(result)).toContain('main frame');
    expect(fixture.getProject).not.toHaveBeenCalled();
    expect(fixture.listCanvasSnapshots).not.toHaveBeenCalled();
    await harness.service.dispose();
  });

  it('rejects an approved restore when its exact native parent changes', async () => {
    const fixture = createStore();
    const harness = createHarness(fixture.store);
    ...
    expect(result).toMatchObject({ ok: false, error: { code: 'OPERATION_FAILED' } });
    expect(ipcErrorMessage(result)).toContain('window changed');
    expect(fixture.restoreCanvasSnapshot).not.toHaveBeenCalled();
    await harness.service.dispose();
  });

  it('resumes a prepared import without mutating data when its owner closes after quiescing', async () => {
    const fixture = createStore();
    const event = liveEvent(46);
    const beforeImport = vi.fn(() => {
      event.destroy();
      return Promise.resolve();
    });
...
function createHarness(
  ...
  return {
    ...
    hooks,
    replaceParent: () => {
      currentParent = { isDestroyed: () => false } as BrowserWindow;
    },
  };
}
