...
describe('PersistedAgentRunContextResolver', () => {
  it('shares video identity and access instructions without copying the video into context', async () => {
    ...
    writeFileSync(path.join(root, 'walkthrough.mp4'), Buffer.alloc(5 * 1024 * 1024, 7));
    ...
    expect(result.context.attachments[0]?.path).toContain('.forgeboard-context');
    expect(result.context.generatedArtifacts?.[0]?.content).toContain(
      'Project-relative path: walkthrough.mp4',
    );
    expect(result.context.generatedArtifacts?.[0]?.content).toContain('SHA-256:');
...
function videoNode(id: string, relativePath: string): CanvasDocument['nodes'][number] {
  return {
    id,
    type: 'video',
    position: { x: 0, y: 0 },
