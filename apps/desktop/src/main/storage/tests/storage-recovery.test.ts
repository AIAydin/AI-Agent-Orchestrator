...
describe('LocalStore persistence and recovery', () => {
  it('upgrades a version-two database in place without losing project or canvas data', () => {
    ...
    legacy.exec(`
      ...
      ALTER TABLE audit_events RENAME TO audit_events_with_hashes;
      ...
      INSERT INTO audit_events(sequence, occurred_at, category, action, outcome, metadata_json)
        SELECT sequence, occurred_at, category, action, outcome, metadata_json
        FROM audit_events_with_hashes;
      DROP TABLE audit_events_with_hashes;
