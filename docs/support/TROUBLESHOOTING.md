...
Start with the exact message shown in the app. Artemis intentionally blocks stale, ambiguous, or
...
Artemis marks the project **Missing folder** and will not redirect its saved canvas to a similar
...
Artemis allocates loopback ports from the range under **Settings → Git & previews**. A collision
...
Artemis does not claim a preview is running until readiness succeeds, and failed reservations are
...
- Artemis reports the conflicting paths and leaves Git's real conflict state visible. It does not
...
- Choose or reset that application under **Settings → Git & previews → External application**. If a
  ...
  it again. Artemis fingerprints the reviewed executable; for a macOS bundle it also verifies the
...
- If you do not want to continue the conflicted operation, use Git recovery outside Artemis only
  if you understand the repository state. Artemis currently has no visual abort/resolution wizard.
...
- **Non-fast-forward:** the remote has work you do not have. Artemis will not force-push. Bring the
...
Artemis preserves bounded pending metadata intent. A same-field conflict can pause for review;
...
Portable import accepts a bounded Artemis JSON export, not an arbitrary project archive.
...
- Select the file again if it changed after review. Artemis binds approval to its exact bytes,
...
## Artemis cannot open its database
...
2. Choose a known Artemis SQLite backup from the configured backup location.
...
Artemis refuses a malformed, foreign, newer-schema, changed, linked, or unsafe backup. If no
...
- On Windows, the directory must have a private ACL; on macOS/Linux it must satisfy the local private
  storage checks. Artemis fails closed rather than publishing a readable backup.
- If complete local-data deletion reports missing recorded backup files, cancel and reconnect their
  ...
  Artemis and will no longer be tracked.
...
- **Lost:** Artemis cannot prove the child process is alive or that termination completed. It does
...
**Send “continue”** is ordinary literal input. Artemis currently has no portable same-process
...
- Artemis version and operating system
