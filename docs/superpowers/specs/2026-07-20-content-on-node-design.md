...
- **Node faces:** `web-preview` face = URL/status strip (back/forward/reload/open-external, dev-server start/stop from the existing previews IPC) + `<webview>` filling the body. `mobile-preview` face = same, wrapped in the device frame at preset logical size, CSS-scaled to fit the node. Both keep `nowheel nodrag` semantics like the agent terminal.
