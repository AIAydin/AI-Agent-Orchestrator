...
Artemis is intended to be distributed through
[GitHub Releases](https://github.com/AIAydin/Artemis/releases), but this repository is
...
| System               | Download                                  |
| -------------------- | ----------------------------------------- |
| macOS, Apple Silicon | `Artemis-<version>-mac-arm64.dmg`         |
| macOS, Intel         | `Artemis-<version>-mac-x64.dmg`           |
| Windows, x64         | `Artemis-<version>-windows-x64-setup.exe` |
| Linux, x64 portable  | `Artemis-<version>-linux-x86_64.AppImage` |
...
```bash
shasum -a 256 Artemis-<version>-mac-arm64.dmg
grep 'Artemis-<version>-mac-arm64.dmg$' SHA256SUMS-darwin-arm64.txt
```

Windows PowerShell:

```powershell
(Get-FileHash .\Artemis-<version>-windows-x64-setup.exe -Algorithm SHA256).Hash.ToLower()
Select-String 'Artemis-<version>-windows-x64-setup.exe$' .\SHA256SUMS-win32-x64.txt
```

Linux:

```bash
sha256sum Artemis-<version>-linux-x86_64.AppImage
grep 'Artemis-<version>-linux-x86_64.AppImage$' SHA256SUMS-linux-x64.txt
```
...
- **macOS:** open the DMG and copy `Artemis.app` to Applications. The ZIP can be extracted and
  moved there instead.
- **Windows:** open the assisted installer. It defaults to a per-user installation and allows an
  optional destination choice; uninstalling does not intentionally delete Artemis's local data.
