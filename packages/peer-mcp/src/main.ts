...
if (!url || !token) {
  process.stderr.write(
    "forgeboard-peer-mcp: FORGEBOARD_PEER_URL/FORGEBOARD_PEER_TOKEN missing\n",
  );
  process.exit(1);
}
...
const hub: HubClient = {
  ...
  screen: (agent) =>
    call(`/v1/screen?agent=${encodeURIComponent(agent)}`) as never,
  ...
  screenshotPreview: (previewId) =>
    call(
      `/v1/preview/screenshot?previewId=${encodeURIComponent(previewId)}`,
    ) as never,
